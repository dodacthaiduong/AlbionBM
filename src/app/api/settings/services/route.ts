import { updateServices } from '@/lib/settings';
...
export async function PUT(request: Request) {
    ...
    if (!items) {
        return jsonError('Invalid services payload');
    }
    const next = await updateServices(items);
    return jsonOk({ services: next.services });
}
